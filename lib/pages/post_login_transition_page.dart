...
class PostLoginTransitionPage extends StatefulWidget {
  ...
  const PostLoginTransitionPage({required this.destination, super.key});
    ...
    return Stack(
      children: [
        ...
        if (_overlayVisible)
          Positioned.fill(
            child: IgnorePointer(
              child: _PostLoginOverlay(controller: _controller),
            ),
          ),
      ],
    );
...
class _PostLoginOverlay extends StatelessWidget {
  ...
  const _PostLoginOverlay({required this.controller});
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final overlayOpacity =
            1 - Curves.easeOutCubic.transform(const Interval(0.44, 1).transform(t));
        ...
        final ballScale = (lerpDouble(0.78, 1, ballEnter)! -
                lerpDouble(0, 0.18, ballExit)!)
            .clamp(0.0, 1.2);
